...
def main():
    ...
    if windows:
        ...
        if soon:
            ...
            for st, d in soon[:4]:
                wmsg += f"• <b>{esc(d['title'])}</b>\n   {st['text']}\n\n"
        wmsg += "💡 신청 기간이 공고된 복지만 추린 목록입니다."
        send_message(wmsg)
    else:
        send_message("📅 <b>지금 신청할 수 있는 복지</b>\n"
                     "━━━━━━━━━━━━━━━━━━━━\n\n"
                     "오늘은 접수 기간이 열려 있는 복지가 없습니다.\n\n"
                     "ℹ️ 중앙부처 복지는 대부분 <b>상시 신청</b>이라 따로 기간이 없습니다. "
                     "기간이 정해진 복지(청년월세·청년내일저축계좌 등)는 접수가 열리면 여기에 표시됩니다.")
